...
class AlamofireServiceClient: ServiceClient {
    ...
    func getBooks(by author: Author?, callback: @escaping ([Book]?,
        ServiceError?) -> Void) {
        
        GETRequest(type: Book.self
            , params: ["where": "author.name like '%\(author?.name ?? "")%'"
    ...
    func GETRequest<T: BaseDTO>(
        type: T.Type, params: Parameters,
        callback: @escaping
            ([T]?, ServiceError?) -> Void
        ){
        ...
        let url = "https://api.backendless.com/00F19476-387E-3131-FFD6-EB5F1F72E100/38659E4B-1D33-06C9-FF05-272C81B2D200/data/"
